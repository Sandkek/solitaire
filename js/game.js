...
//создание начальных стопок карт
function makePile(pile, num) {
    for (var i = 0; i < num; i++)
    pile.unshift(deck.shift());
}

//клик на пустые ячейки
...
//проверка ходов на игровом блоке
...
//возвращение поля в нормальное состояние после недоступного хода
function illegal() {
    resetFirstPile();
    render();
}

//переворачивает колоду на доску когда просмотрены все карты
function reloadDeck() {
    if (deck.length === 0) {
        ...
        if (finalPile2[0] === staticCards[12] || finalPile2[0] === staticCards[25] || finalPile2[0] === staticCards[38] || finalPile2[0] === staticCards[51]) {
            if (finalPile3[0] === staticCards[12] || finalPile3[0] === staticCards[25] || finalPile3[0] === staticCards[38] || finalPile3[0] === staticCards[51]) {
                if (finalPile4[0] === staticCards[12] || finalPile4[0] === staticCards[25] || finalPile4[0] === staticCards[38] || finalPile4[0] === staticCards[51]) {
                    alert("Вы выиграли!\nВам потребовалось: " + moves + " ходов");

                    saveGameResult(moves);
                    
                    //$('.winMessage').text('Вы выиграли!');
                    //$('.winMessage').text('Вы выиграли со счетом: ' + (score * moves /100));
                    // можно сделать алертом
                    //$('.score').text('Счет: ' + score * moves /100)
                }
            }
        }
    }
...
//отображение карт в основании и колоде
function renderTop() {
    if (deck.length > 0) {
        $('.deck').addClass('card').addClass('back');
    }
    else $(".deck").removeClass('back');
    if (deckPile.length > 0) {
...
//удаляет лишние классы из конечных стопок
...
//отображение карт в игровом блоке
...
//Переворачивает первую карту в каждой стопке
function flipFirstCard() {
    if (boardPile1.length > 0); {
        $(boardState1[boardPile1.length-1]).addClass('faceUp').removeClass('back')}
    if (boardPile2.length > 0); {
        $(boardState2[boardPile2.length-1]).addClass('faceUp').removeClass('back')}
    if (boardPile3.length > 0); {
        $(boardState3[boardPile3.length-1]).addClass('faceUp').removeClass('back')}
    if (boardPile4.length > 0); {
        $(boardState4[boardPile4.length-1]).addClass('faceUp').removeClass('back')}
    if (boardPile5.length > 0); {
        $(boardState5[boardPile5.length-1]).addClass('faceUp').removeClass('back')}
    if (boardPile6.length > 0); {
        $(boardState6[boardPile6.length-1]).addClass('faceUp').removeClass('back')}
    if (boardPile7.length > 0); {
        $(boardState7[boardPile7.length-1]).addClass('faceUp').removeClass('back')}};

//переворачивает карты которые находятся под верхней в игровом блоке
...
//Если что-то на плате не имеет другого класса, то карта отображается как перевернутая карта
function renderFaceDowns() {  
for (var i = 0; i < boardPile1.length; i++)
    if ($(boardState1[i]).hasClass('faceUp' || 'empty' || 'back')) {}
    else {$(boardState1[i]).addClass('back')}
for (var i = 0; i < boardPile2.length; i++)
    if ($(boardState2[i]).hasClass('faceUp' || 'empty' || 'back')) {}
    else {$(boardState2[i]).addClass('back')}
for (var i = 0; i < boardPile3.length; i++)
    if ($(boardState3[i]).hasClass('faceUp' || 'empty' || 'back')) {}
    else {$(boardState3[i]).addClass('back')}
for (var i = 0; i < boardPile4.length; i++)
    if ($(boardState4[i]).hasClass('faceUp' || 'empty' || 'back')) {}
    else {$(boardState4[i]).addClass('back')}
for (var i = 0; i < boardPile5.length; i++)
    if ($(boardState5[i]).hasClass('faceUp' || 'empty' || 'back')) {}
    else {$(boardState5[i]).addClass('back')}
for (var i = 0; i < boardPile6.length; i++)
    if ($(boardState6[i]).hasClass('faceUp' || 'empty' || 'back')) {}
    else {$(boardState6[i]).addClass('back')}
for (var i = 0; i < boardPile7.length; i++)
    if ($(boardState7[i]).hasClass('faceUp' || 'empty' || 'back')) {}
    else {$(boardState7[i]).addClass('back')}
}

// Присваивает класс и связанное с ним изображение на основе данных массива
function renderCard(pile, num) {
    ...
    if (num === undefined) {
        ...
        $(pile).removeClass('empty')
    }
}

//сбрасывает переменную firstPile
...
//нормализация непрозрачности
function makeFaceUpSolid() {
    $('.faceUp').not('.empty').css('opacity', '1');
    $('.empty').css('opacity', '0');
